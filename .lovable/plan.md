...
3. **AI reader becomes a fallback, only when the structural reader finds nothing.** Never a merge of the two, never a tie-break: a page either reads structurally or falls back whole. Its names keep the existing check against the page text.
4. **Record which reader read each roster**, on every player row and on the composition summary, so the fallback's value is measurable: how often it runs, on which schools, and whether the rosters it produces survive review. That number decides whether it stays.
5. **Give coaches the same shape.** The crawl calls the tested coach reader for coach pages; the model's two-field ask is the fallback when it finds no staff at all, and is recorded the same way.
6. **Guard the composition summary, same rules as player rows.** No source page or a domain another school holds means refused and parked for review, not written. A suspicious read is written but marked suspect, and suspect rows are withheld from every display and from the dashboard count — visible only in the review screens, labelled.
7. **Test the crawl's own path, not just the reader.** Two readers were able to coexist precisely because nothing tested the path the crawl takes. The saved pages are fed through the crawl's entry point, with the network and the model both stubbed, and the test asserts the structural reader ran, the model was never called, the fallback fires only on a page with nothing structural to read, and the resulting counts match the recorded baseline. Report per saved page: players, coaches, columns published vs captured, which reader ran.
