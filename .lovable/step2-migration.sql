-- STEP 2A — proposed, NOT YET RUN.
-- 1. Compare roster/staff pages on the full normalized address (host + path),
--    so two different pages on one shared platform domain never collide.
-- 2. Extend the collision guard to universities.website_url.
-- 3. Columns the identity-bound discovery needs: the institution's own website
--    and two-year/four-year level from the federal directory, plus somewhere to
--    record the evidence that justified a match.

-- Full page key: lowercase host without www, path kept, query/fragment and
-- trailing slash dropped. "Site.edu/Sports/Baseball/roster/?x=1" and
-- "site.edu/sports/baseball/roster" are the same page.
...
DECLARE
  ...
  holder record;
BEGIN
  -- Athletics domain: one registrable domain, one institution (platforms exempt).
  ...
  -- Roster page: full address, one institution. Platform domains are NOT exempt
  -- here, because the same full address really can only be one school's page.
...
-- New: the same rule on the school record's own website field.
...
-- Indexes matching the new comparison keys.
