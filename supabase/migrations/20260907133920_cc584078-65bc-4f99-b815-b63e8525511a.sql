-- lovable-cron-fallback-reviewed: 1440 runs/day; restores the pre-existing unattended collection runner schedule that the buggy trigger unscheduled; the run is a long-lived bounded queue drain that must keep making passes without a person on the page
...
BEGIN
  ...
  -- Anything still to do at ANY level, including work set aside for a later
  -- level and work part-way through, counts as outstanding.
...
BEGIN
  ...
  -- Free part-way work whatever the run state, so a later start isn't blocked.
  _freed := public.reclaim_stale_leases(15);

  IF _row.id IS NULL OR NOT _row.is_running OR _row.stop_requested THEN
    RETURN jsonb_build_object('running', false, 'freed', _freed, 'retried', 0, 'rearmed', false);
  END IF;
...
UPDATE public.ingest_queue
   SET status = 'pending', leased_at = NULL, attempts = 0, updated_at = now()
...
UPDATE public.collection_state
   SET is_running = true,
       ...
       last_beat_at = now(),
       last_message = 'Restarted after a false finish - continuing',
       updated_at = now()
 WHERE id = 'singleton';

SELECT cron.unschedule('power-recruit-collection')
 WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'power-recruit-collection');

SELECT cron.schedule('power-recruit-collection', '* * * * *',
  $inner$SELECT public.trigger_collection_runner();$inner$);